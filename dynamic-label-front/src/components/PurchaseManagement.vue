  ...
  <div class="purchase-management">
    ...
    <div class="main-content">
      ...
      <div class="filters-section">
        <div class="filters">
          <div class="filter-group">
            <label>状态</label>
            <select v-model="filters.status" @change="loadPurchaseOrders">
          ...
          <div class="filter-group">
            <label>供应商</label>
            <input v-model="filters.supplier_name" @input="debounceSearch" placeholder="输入供应商名称">
          </div>
          <div class="filter-group">
            <label>订单号</label>
            <input v-model="filters.po_number" @input="debounceSearch" placeholder="输入采购订单号">
          </div>
      ...
      <div class="orders-section">
        ...
        <!-- 订单卡片列表 -->
        <div class="orders-grid" v-if="purchaseOrders.length > 0">
          <div 
          ...
          >
            <div class="card-header">
              <div class="order-info">
                <h3 class="order-number">{{ order.po_number }}</h3>
                <span class="supplier-name">{{ order.supplier_name }}</span>
              </div>
            ...
            <div class="card-actions">
              ...
              <button 
                ...
                @click.stop="generateReceivingTasks(order)"
      ...
      <div class="dialog" @click.stop>
        ...
        <div class="dialog-body">
          <div class="form-group">
            <label>供应商名称 *</label>
            <input v-model="newOrder.supplier_name" placeholder="输入供应商名称" required>
          </div>
          <div class="form-group">
            <label>供应商ID</label>
            <input v-model="newOrder.supplier_id" placeholder="输入供应商ID">
          </div>
          <div class="form-group">
            <label>预期到货日期</label>
            <input v-model="newOrder.expected_date" type="date">
          </div>
          <div class="form-group">
            <label>优先级</label>
            ...
            <div class="order-lines">
              <div v-for="(line, index) in newOrder.lines" :key="index" class="order-line">
                <div class="line-fields">
                  ...
                  <input v-model.number="line.quantity" type="number" placeholder="数量" class="qty-input">
        ...
        <div class="dialog-actions">
          ...
          <button @click="createPurchaseOrder" :disabled="creating" class="confirm-btn">
            {{ creating ? '创建中...' : '创建订单' }}
          </button>
        </div>
      </div>
...
const loadPurchaseOrders = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      purchaseOrders.value = data.data.purchaseOrders
      pagination.value = data.data.pagination
    } else {
      console.error('获取采购订单失败')
    }
  } catch (error) {
...
const createPurchaseOrder = async () => {
  if (!newOrder.supplier_name || newOrder.lines.length === 0) {
    alert('请填写供应商名称和至少一行采购明细')
    return
  }
...
const confirmOrder = async (order) => {
  if (!confirm(`确认采购订单 ${order.po_number}？`)) return
...
const generateReceivingTasks = (order) => {
  // TODO: 实现生成收货任务逻辑
  console.log('生成收货任务:', order)
  alert('收货任务功能开发中...')
}
